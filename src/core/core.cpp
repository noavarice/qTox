                         ...
                         size_t length, void* vCore)
{
    ...
    QByteArray pk(static_cast<const char*>(static_cast<const void*>(data)), length);
    const GroupInvite inviteInfo{friendId, type, pk};
    switch (type) {
 ...
 * @param inviteInfo Object that contains information about group invitation
 *
 * @return Conference number on success, UINT32_MAX on failure.
 */
uint32_t Core::joinGroupchat(const GroupInvite& inviteInfo) const
{
    const uint8_t type = inviteInfo.getType();
    ...
    const uint16_t length = invite.length();
    switch (type) {
    case TOX_CONFERENCE_TYPE_TEXT: {
        qDebug() << QString("Trying to join text groupchat invite sent by friend %1").arg(friendId);
        TOX_ERR_CONFERENCE_JOIN error;
        uint32_t groupId = tox_conference_join(tox, friendId, cookie, length, &error);
    ...
    case TOX_CONFERENCE_TYPE_AV: {
        ...
        return toxav_join_av_groupchat(tox, friendId, cookie, length, CoreAV::groupCallCallback,
